...
// Get all applications for company's jobs(use it for dashboard)
jobRouter.get("/api/applications/my-jobs", auth, async (req, res) => {
  try {
    ...
    const applications = await Application.find({ jobId: { $in: jobIds } })
      ...
      .populate({
        path: "candidateId",
        select: "firstName lastName email skills gender profilePicture",
      })
      .sort({ createdAt: -1 });
...
jobRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      // In your existing review API, add this after updating the application status:
      application.status = status;
      application.reviewedAt = new Date();
      application.notificationSent = true; // Mark that notification should be sent
      application.notificationRead = false; // Student hasn't read it yet
      await application.save();
